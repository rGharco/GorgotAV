...
#define PE32_PLUS_SIGNATURE 0x20b
...
PEStatus parse_pe(PFileContext fc) {
	...
	if (Magic == PE32_PLUS_SIGNATURE) {
		...
		WORD nrOfSections = nt32->FileHeader.NumberOfSections;

		PIMAGE_SECTION_HEADER ptrToSectionsStart = (PIMAGE_SECTION_HEADER)((BYTE*)nt32 + sizeof(IMAGE_NT_HEADERS32));
